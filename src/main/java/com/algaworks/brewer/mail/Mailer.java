...
public class Mailer {
	...
	public void enviar(Venda venda) {
		...
		for (ItemVenda item : venda.getItens()) {
			if (item.getCerveja().temFoto()) {
				String cid = "foto-" + item.getCerveja().getCodigo();
				context.setVariable(cid, cid);

				fotos.put(cid, item.getCerveja().getFoto() + "|" + item.getCerveja().getContentType());
			} else {
		...
		try {
			...
			helper.setFrom("brcabral@gmail.com");
			helper.setTo(venda.getCliente().getEmail());
			helper.setSubject(String.format("Brewer - Compra nº %d finalizada.", venda.getCodigo()));
